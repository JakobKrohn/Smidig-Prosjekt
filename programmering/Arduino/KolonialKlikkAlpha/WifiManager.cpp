...
// Tries to connect to selected ssid with password
bool WifiManager::connect(String ssid, String password)
{
  ...
  {
    delay(500);
    if (attempts++ >= CONNECTION_TIMEOUT) break;
  }
...
//  - Start an access point for user to input wifi ssid and password
//  - Returns false if the access point has been up for more than 20 minutes
//      to save battery
//  - Returns true if the ssid and password was received
bool WifiManager::startAccessPoint()
{
  ...
  _webServer->on("/", std::bind(&WifiManager::root, this));
  _webServer->on("/generate_204", std::bind(&WifiManager::root, this)); // Android captive portal
  _webServer->on("/fwlink", std::bind(&WifiManager::root, this));       // Windows captive portal
  _webServer->on("/reply", std::bind(&WifiManager::reply, this));
  _webServer->begin();
  ...
  while (1)
  {
    ...
    if (millis() > 1200000) return false;
    else if (_fetchedInput) return true;
  }
}
...
void WifiManager::scanWifi()
{
  ...
  if (_networksFound < 1)
  {
    // We could not find any networks
    return;
  }

  // Add found networks to list
  for (int i = 0; i < _networksFound; i++)
  {
    _scannedNetworks[i] = i;
  }
  ...
  for (int i = 0; i < _networksFound; i++)
  {
    for (int j = 0; j < i; j++)
    {
      if (WiFi.RSSI(_scannedNetworks[j]) > WiFi.RSSI(_scannedNetworks[i]))
      {
        std::swap(_scannedNetworks[i], _scannedNetworks[j]);
      }
    }
  }
  ...
  for (int i = 0; i < _networksFound; i++)
  {
    if (_scannedNetworks[i] == -1) continue;

    String ssid = WiFi.SSID(_scannedNetworks[i]);

    for (int j = i + 1; j < _networksFound; j++)
    {
      if (ssid == WiFi.SSID(_scannedNetworks[j]))
      {
        _scannedNetworks[j] = -1;
      }
    }
  }
}
...
void WifiManager::root()
{
  ...
  for (int i = 0; i < _networksFound; ++i)
  {
    ...
    rootPage += WiFi.SSID(_scannedNetworks[i]);
    rootPage += "'>";
    rootPage += WiFi.SSID(_scannedNetworks[i]);
    rootPage += "</option>";
  }
  ...
  rootPage += "<input name='ssid' type='text' id='ssid' value='' style='display: none; ' required>";
  ...
  _webServer->send(200, "text/html", rootPage);
}

void WifiManager::reply()
{
  String replyPage = String();

  String newSSID = _webServer->arg("ssid");
  String newPASS = _webServer->arg("pass");

  _SSID = _webServer->arg("ssid");
  _password = _webServer->arg("pass");

  if (newSSID.length() > 0) _fetchedInput = true;
  else _fetchedInput = false;
  ...
  replyPage += "<style>";
  replyPage += "div {width:50%; position: relative; margin:auto; padding-top:12%; font-weight:300;}";
  replyPage += "li {padding-left:30px;}";
  replyPage += "a {color: inherit;}";
  replyPage += "footer {text-align:center;}";
  replyPage += "svg {width: 200px}";
  ...
  _webServer->send(200, "text/html", replyPage);
}
